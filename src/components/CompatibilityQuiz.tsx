...
import { calculatePetMatch } from "@/utils/quizUtils";
...
const CompatibilityQuiz = () => {
  ...
  const handleNext = () => {
    ...
    if (step < totalSteps) {
      ...
      const matchResults = calculatePetMatch(answers);
          ...
          <div className="space-y-6">
            {results.map((result, index) => (
              ...
              >
                <div className="flex items-center gap-3 mb-2">
                  {result.type === "dog" && <Dog className="h-6 w-6 text-purple-600" />}
                  {result.type === "cat" && <Cat className="h-6 w-6 text-purple-600" />}
                  {result.type === "rabbit" && <Rabbit className="h-6 w-6 text-purple-600" />}
                  <h3 className="text-lg font-semibold">
                    {result.type === "dog" ? "Chien" : result.type === "cat" ? "Chat" : "Lapin"}
                    {index === 0 && " (Meilleure correspondance)"}
                  </h3>
                  <span className="ml-auto text-sm text-gray-500">
                    Compatibilité: {Math.round((result.score / 20) * 100)}%
                  </span>
                </div>
                
                <div className="mt-2">
                  <h4 className="text-sm font-medium text-gray-700 mb-1">Pourquoi cette recommandation :</h4>
            ...
            <div className="text-center mt-6">
              <p className="text-sm text-gray-600 mb-4">
                Ces résultats sont basés sur vos réponses et représentent les meilleures correspondances pour votre style de vie.
              </p>
              <Button 
